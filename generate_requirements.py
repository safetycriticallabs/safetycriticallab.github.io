...
.req-toc {{ list-style: none; padding: 0; margin: 0 0 56px 0; display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 6px 18px; }}
.req-toc li {{ font-size: 14px; }}
.req-toc a {{ color: var(--ink); text-decoration: none; border-bottom: 1px solid transparent; }}
.req-toc a:hover {{ color: var(--blue); border-bottom-color: rgba(46,109,180,0.3); }}
...
.req-area {{ scroll-margin-top: 90px; }}   /* the area anchors (#ai-N) are on the section, so it clears the fixed nav too */
...
<div class="inner-hero">
  ...
  <div class="inner-hero-inner">
    ...
    <p class="inner-hero-lead">
      The complete text of the AI Requirements Framework, version {esc(version)}, deposited
      {esc(published)}, with this page regenerated from the corpus on {esc(updated)}. Every
      requirement is published with its rationale, the method by which it is
